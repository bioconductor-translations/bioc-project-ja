...
このレッスンは`r R.version.string`を使用して開発およびテストされました。

RStudioを起動し、あなたが`r r_version_string.patch_x()`のRバージョンを使用しているか確認してください。xは任意のパッチバージョン、例えば`r r_version_string()`です。
